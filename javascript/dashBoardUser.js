...
const tab_mail_small = [
  document.querySelector(".categorie_small"),
  document.querySelector(".contenu_small"),
  document.querySelector(".auteur_small"),
];
...
for (let i = 0; i < annoncesUl.length; i++) {
  annoncesUl[i].addEventListener("click", () => {
    ...
    for (let k = 0; k < tab_mail_small.length; k++) {
      tab_mail_small[k].style.display = "block";
    }
...
if (closeButton) {
  closeButton.addEventListener("click", () => {
    if (AnnoncesFullScreen == true) {
      ...
      for (let k = 0; k < tab_mail_small.length; k++) {
        tab_mail_small[k].style.display = "none";
      }
